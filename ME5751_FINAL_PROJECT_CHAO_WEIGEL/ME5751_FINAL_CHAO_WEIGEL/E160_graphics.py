...
class E160_graphics:
	def __init__(self, environment):
		...
		self.left_speed = Label(self.tk,text = "right wheel angle")
		...
		#self.environment.robots[0].controller.add_goal_points(self.path.path)
	...
	def set_velocity_disp(self, robot):
		self.left_speed.config(text="right wheel angle "+str(robot.state.phi_l))
		self.right_speed.config(text="right wheel speed "+str(robot.state.phi_r))
		
		if(abs(robot.state.phi_l)>40.0 or abs(robot.state.phi_r)>20.0):
			self.impossible+=1
